...
public class RegionService {
    ...
    public void deleteRegion(Long id) {
        Region region = repository.findById(id).orElseThrow(() -> new IllegalStateException("Region not found"));
        repository.delete(region);
    }

    public RegionDTO updateRegion(RegionDTO updateDTO) {
        Region region = repository.findById(updateDTO.getId()).orElseThrow(() -> new IllegalStateException("Region not found"));
